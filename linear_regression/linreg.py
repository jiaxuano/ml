...
def sigmoid(z):
    return 1/(1+np.exp(-z)) # preserves the dimension of input array
...
def minimize(X, y, loss_gradient,
    ...
    h = np.zeros(p).reshape(p,1) #I changed from the original h = np.zeros(p)
    for i in list(range(max_iter)):
        # We need to calculate and store the past square gradient
        grad = loss_gradient(X,y,B,lmbda)
        if np.linalg.norm(grad) < precision:
            break
        h += grad**2
        B -= (eta*grad/np.sqrt(h+eps))
    return B
...
class RidgeRegression621: # MODIFY THIS ONE
    ...
    def fit(self, X, y):
        ...
        self.B0_arr = np.array([[np.mean(y)]])
        self.B = np.vstack((self.B0_arr, self.B))
        self.B0 = np.mean(y)

        # LOG: 3rd try, only minimize p-1 of X, dim(B) is dependent on X, not working
        # LOG: 4th try, use y = XB + B0, using B0 as a num instead of vector
        # LOG: finally chose to concatenate B0 vector with rest of B matrix, and add numerical B0 value when predicting
        # LOG: All tests passed with this method.
